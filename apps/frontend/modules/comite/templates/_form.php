...
<form class="form-horizontal"
      action="<?php echo url_for('comite/' . ($form->getObject()->isNew() ? 'create' : 'update') . (!$form->getObject()->isNew() ? '?id_comite=' . $form->getObject()->getIdComite() : '')) ?>"
    ...
    <div class="box-footer">
        &nbsp;<a class="btn btn-default" style="margin:5px;" title="" href="<?php echo url_for('comite/index') ?>">Regresar</a>
        <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('<span>Borrar</span>', 'comite/delete?id_comite=' . $form->getObject()->getIdComite(), array('method' => 'delete', 'class' => 'btn btn-danger', 'style' => 'margin: 5px;', 'confirm' => 'Desea borrar el registro?')) ?>
        <?php endif; ?>
