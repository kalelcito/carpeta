...
<form class="form-horizontal"
      action="<?php echo url_for('usuarios/' . ($form->getObject()->isNew() ? 'create' : 'update') . (!$form->getObject()->isNew() ? '?id_usuario=' . $form->getObject()->getIdUsuario() : '')) ?>"
    ...
    <div class="box-footer">
        &nbsp;<a class="btn btn-default" style="margin:5px;" title="" href="<?php echo url_for('usuarios/index') ?>">Regresar</a>
        <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('<span>Borrar</span>', 'usuarios/delete?id_usuario=' . $form->getObject()->getIdUsuario(), array('method' => 'delete', 'class' => 'btn btn-danger', 'style' => 'margin: 5px;', 'confirm' => 'Desea borrar el registro?')) ?>
        <?php endif; ?>
